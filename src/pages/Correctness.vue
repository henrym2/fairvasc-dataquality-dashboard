  ...
  <v-container>
    <v-row>
      <v-col>
        <Widget title="Average Correctness of Data Over Time" subtitle="Average total correctness of data over time" :id=0 @expand="maximise">
          <line-chart :data="filterReg(correctnessOverTime)" :download="true"></line-chart>
        </Widget>
      </v-col>
      <v-col>
        <Widget title="Average total correctness" subtitle="Average total correctness of the current dataset" :id=1 @expand="maximise">
            <pie-chart :data="averageAll(filterReg(correctnessData))" :download="true"></pie-chart>
        </Widget>
      </v-col>
    </v-row>
    <v-row>
      <v-col>
        <Widget title="Correctness percentage per field" subtitle="Percentage of correct data per field in the dataset" :id=2 @expand="maximise">
          <template v-slot:controls>
            ...
            <column-chart :data="filterVals(filterReg(correctnessData))" :download="true"></column-chart>
        </Widget>
      </v-col>
    </v-row>
     <v-dialog
    ...
    >
      ...
      >
        ...
        <v-divider/>
            <line-chart v-if="maximised.id == 0" :data="filterReg(correctnessOverTime)" :legend="true" suffix="%" :download="true" height="70vh"></line-chart>
            <pie-chart v-if="maximised.id == 1" :data="averageAll(filterReg(correctnessData))" :legend="true" suffix="%" :download="true"></pie-chart>
            <column-chart v-if="maximised.id == 2" :data="filterVals(filterReg(correctnessData))" suffix="%" :download="true" height="70vh"></column-chart>
        <v-divider/>
          <v-card-actions>
